<article class="card card-apelos p-2 my-3" data-id={{ $apelo->id }}>
    ...
    <div>
        ...
        @endcan
        @can ('admin',App\Utilizador::class)
        <form method = "POST" action="{{route('admin-desbloqueia',$apelo->criador->utilizador->id)}}" id="bloqueia-utilizador-form">
        {{ csrf_field() }}
        @method('DELETE')
        <button type="submit" class="btn questao-button admin-apelo-acao desbloquear btn-sm float-end m-2">Desbloquear</button>
        </form>
        @endcan
    </div>
</article>
